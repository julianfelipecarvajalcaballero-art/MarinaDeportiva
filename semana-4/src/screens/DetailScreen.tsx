...
export default function DetailScreen() {
  ...
  const handleToggleSave = () => { isSaved ? removeItem(item.id) : addItem(item); };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <Pressable
          ...
          style={({ pressed }) => [
            styles.saveButton, isSaved && styles.savedButton, pressed && styles.saveButtonPressed,
          ]}
        >
          <Ionicons name={isSaved ? 'bookmark' : 'bookmark-outline'} size={20} color={COLORS.white} />
...
const styles = StyleSheet.create({
  ...
  boatName: { ...TYPOGRAPHY.header, color: COLORS.textPrimary, marginBottom: SPACING.sm },
  infoRow: {
    flexDirection: 'row', alignItems: 'center', gap: SPACING.md,
    backgroundColor: COLORS.card, padding: SPACING.md, borderRadius: 12,
  ...
  saveButton: {
    ...
    backgroundColor: COLORS.primary, paddingVertical: SPACING.md, borderRadius: 12, marginTop: SPACING.md,
  },
  savedButton: { backgroundColor: COLORS.error },
  saveButtonPressed: { opacity: 0.8 },
  saveButtonText: { ...TYPOGRAPHY.subtitle, color: COLORS.white },
});
